...
const control_frequency = 1 # in seconds
...
const interval_downtime_event_mean = 15 # seconds
const interval_downtime_event_dev = 10 #  seconds

## possibility of multiple machines going down in parallel and at overlapping times
const number_parallel_downtime_events = 6
...
type SimState {
    ...
    all_conveyor_levels_estimate: number[11], # estimated levels of the conveyors
...
function machine6_output(state: SimState) {
    ...
    if state.machines_actual_speed[6] == 0 {
        return 0
    }
    if state.conveyor_infeed_m2_prox_empty[5] == 1 or state.conveyor_discharge_p2_prox_full[6] == 1 {
        var machine6_reward = machine6_throughput - alpha * conveyor_penalty
...
function machine7_output(state: SimState) {
    ...
    if state.machines_actual_speed[7] == 0 {
        return 0
    }
    if state.conveyor_infeed_m2_prox_empty[6] == 1 or state.conveyor_discharge_p2_prox_full[7] == 1 {
        var machine7_reward = machine7_throughput - alpha * conveyor_penalty
...
function machine8_output(state: SimState) {
    ...
    if state.conveyor_infeed_m2_prox_empty[7] == 1 or state.conveyor_discharge_p2_prox_full[8] == 1 {
        var machine8_reward = machine8_throughput - alpha * conveyor_penalty
...
function machine9_output(state: SimState) {
    ...
    if state.machines_actual_speed[9] == 0 {
        return 0
    }
    if state.conveyor_infeed_m2_prox_empty[8] == 1 or state.conveyor_discharge_p2_prox_full[9] == 1 {
        var machine9_reward = machine9_throughput - alpha * conveyor_penalty
...
function machine10_output(state: SimState) {
    ...
    if state.machines_actual_speed[10] == 0 {
        return 0
    }
    if state.conveyor_infeed_m2_prox_empty[9] == 1 or state.conveyor_discharge_p2_prox_full[10] == 1 {
        var machine10_reward = machine10_throughput - alpha * conveyor_penalty
...
function machine11_output(state: SimState) {
    ...
    if state.machines_actual_speed[11] == 0 {
        return 0
    }
    if state.conveyor_infeed_m2_prox_empty[10] == 1 {
        var machine11_reward = machine11_throughput - alpha * conveyor_penalty
        ...
        curriculum {
            ...
            lesson `learn 1` {
                scenario {
                    ...
                    machine6_initial_speed: number<machine_min_speed[6] .. machine_max_speed[6] step 1>,
                    machine7_initial_speed: number<machine_min_speed[7] .. machine_max_speed[7] step 1>,
                    machine8_initial_speed: number<machine_min_speed[8] .. machine_max_speed[8] step 1>,
                    machine9_initial_speed: number<machine_min_speed[9] .. machine_max_speed[9] step 1>,
                    machine10_initial_speed: number<machine_min_speed[10] .. machine_max_speed[10] step 1>,
                    machine11_initial_speed: number<machine_min_speed[11] .. machine_max_speed[11] step 1>,
                    infeed_prox_upper_limit: infeed_prox_upper_limit,
                    infeed_prox_lower_limit: infeed_prox_lower_limit,
                    discharge_prox_upper_limit: discharge_prox_upper_limit,
                    discharge_prox_lower_limit: discharge_prox_lower_limit,
